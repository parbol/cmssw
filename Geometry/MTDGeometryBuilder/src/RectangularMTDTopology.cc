...
void RectangularMTDTopology::setOffset(const int& BIG_PIX_PER_ROC_X,
  ...
  LogDebug("RectangularMTDTopology") << "nrows " << m_nrows << ", ncols " << m_ncols << ", pitchx " << m_pitchx
                                     << ", pitchy " << m_pitchy << ", xoffset " << m_xoffset << ", yoffset "
                                     << m_yoffset << ", BIG_PIX_PER_ROC_X " << BIG_PIX_PER_ROC_X
                                     << ", BIG_PIX_PER_ROC_Y " << BIG_PIX_PER_ROC_Y << ", ROWS_PER_ROC " << ROWS_PER_ROC
                                     << ", COLS_PER_ROC " << COLS_PER_ROC << ", ROCS_X " << m_ROCS_X << ", ROCS_Y "
                                     << m_ROCS_Y << "\nNROWS " << m_ROWS_PER_ROC * m_ROCS_X << ", NCOL "
                                     << m_COLS_PER_ROC * m_ROCS_Y;
}
...
// Transform LocalPoint in cm to measurement in pitch units.
//std::pair<float, float> RectangularMTDTopology::pixel(const LocalPoint& p) const {
std::tuple<float, float, bool> RectangularMTDTopology::pixelFrame(const LocalPoint& p) const {
  // check limits
  ...
  float y_gap = 0.1;
  float x_gap = 0.1;
  float y_gap_sensor = 0.3;
  float x_gap_sensor = 0.3;
...
#define EPS 0
  ...
  if((fractionY > 1.0 - y_gap || fractionY < y_gap) || (iybin == 0 && fractionY < y_gap_sensor) || (iybin == m_ncols - 1 && fractionY > 1.0 - y_gap_sensor)) validPoint = false;  
  ...
  int ixbin = int(newxbin);
  float fractionX = newxbin - ixbin;
  if((fractionX > 1.0 - x_gap || fractionX < x_gap) || (ixbin == 0 && fractionX < x_gap_sensor) || (ixbin == m_nrows - 1 && fractionX > 1.0 - x_gap_sensor)) validPoint = false;  
  ...
  return std::tuple<float, float, bool>(mpX, mpY, validPoint);
}
