// -*- C++ -*-
//
// Package:    MTDAlignmentAlgorithms
// Class:      MTDAlignmentFromReference
//
...
class MTDAlignmentFromReference : public AlignmentAlgorithmBase {
public:
  MTDAlignmentFromReference(const edm::ParameterSet& cfg, edm::ConsumesCollector& iC);
  ~MTDAlignmentFromReference() override;

  void initialize(const edm::EventSetup& iSetup,
                  AlignableTracker* alignableTracker,
                  AlignableMTD* alignableMuon,
  ...
  void processMTDResidualsFromTrack(MTDResidualsFromTrack& mrft);
...
private:
  ...
  void parseReference(align::Alignables& reference,
                      const align::Alignables& all_BTL_modules,
                      const align::Alignables& all_ETL_modules);
  ...
  // tokens
  const edm::ESGetToken<MTDGeometry, MTDGeometryRecord> m_mtdGeometryToken;
  ...
  edm::InputTag m_trackerCollectionTag;
  ...
  void bookNtuple();
  TTree* m_ttree;
  MuonResidualsFitter::MTDAlignmentTreeRow m_tree_row;

  bool m_debug;
};

MTDAlignmentFromReference::MTDAlignmentFromReference(const edm::ParameterSet& cfg, edm::ConsumesCollector& iC)
    : AlignmentAlgorithmBase(cfg, iC),
      m_mtdGeometryToken(iC.esConsumes<edm::Transition::BeginRun>()),
      ...
      m_DetIdToken(iC.esConsumes(edm::ESInputTag("", "MTDDetIdAssociator"))),
...
MTDAlignmentFromReference::~MTDAlignmentFromReference() { delete m_alignableNavigator; }

void MTDAlignmentFromReference::bookNtuple() {
  edm::Service<TFileService> fs;
...
bool MTDAlignmentFromReference::numeric(std::string s) { return s.length() == 1 && std::isdigit(s[0]); }

int MTDAlignmentFromReference::number(std::string s) {
  if (!numeric(s))
    assert(false);
  return atoi(s.c_str());
}

void MTDAlignmentFromReference::initialize(const edm::EventSetup& iSetup,
                                            AlignableTracker* alignableTracker,
                                            AlignableMuon* alignableMuon,
                                            AlignableExtras* extras,
                                            AlignmentParameterStore* alignmentParameterStore) {
  if (alignableMuon == nullptr)
    throw cms::Exception("MTDAlignmentFromReference") << "doMuon must be set to True" << std::endl;
  ...
  if (m_residualsModel == std::string("pureGaussian"))
    ...
    throw cms::Exception("MTDAlignmentFromReference")
...
void MTDAlignmentFromReference::run(const edm::EventSetup& iSetup, const EventInfo& eventInfo) {
  if (m_debug)
...
void MTDAlignmentFromReference::processMuonResidualsFromTrack(MuonResidualsFromTrack& mrft) {
...
void MTDAlignmentFromReference::terminate(const edm::EventSetup& iSetup) {
  bool m_debug = false;
  ...
  if (m_writeTemporaryFile != std::string(""))
    writeTmpFiles();
  if (m_debug)
    std::cout << "end: MTDAlignmentFromReference::terminate()" << std::endl;
}

void MTDAlignmentFromReference::fitAndAlign() {
  bool m_debug = false;

  edm::Service<TFileService> tfileService;
  TFileDirectory rootDirectory(tfileService->mkdir("MTDAlignmentFromReference"));
...
void MTDAlignmentFromReference::readTmpFiles() {
...
void MTDAlignmentFromReference::writeTmpFiles() {
  FILE* file;
...
void MTDAlignmentFromReference::correctBField() {
  bool m_debug = false;
...
void MTDAlignmentFromReference::fiducialCuts() {
  for (std::vector<unsigned int>::const_iterator index = m_indexes.begin(); index != m_indexes.end(); ++index) {
    if (m_debug)
...
void MTDAlignmentFromReference::eraseNotSelectedResiduals() {
  for (std::vector<unsigned int>::const_iterator index = m_indexes.begin(); index != m_indexes.end(); ++index) {
    if (m_debug)
...
void MTDAlignmentFromReference::selectResidualsPeaks() {
...
std::string MTDAlignmentFromReference::chamberPrettyNameFromId(unsigned int idx) {
  DetId id(idx);
  char cname[40];
  if (id.subdetId() == MuonSubdetId::DT) {
    DTChamberId chamberId(id.rawId());
...
void MTDAlignmentFromReference::fillNtuple() {
...
void MTDAlignmentFromReference::parseReference(align::Alignables& reference,
...
DEFINE_EDM_PLUGIN(AlignmentAlgorithmPluginFactory, MTDAlignmentFromReference, "MTDAlignmentFromReference");
