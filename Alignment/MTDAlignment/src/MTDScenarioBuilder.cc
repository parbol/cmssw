...
MTDScenarioBuilder::MTDScenarioBuilder(Alignable* alignable)
  ...
  theAlignableMTD = dynamic_cast<AlignableMuon*>(alignable);
...
void MTDScenarioBuilder::applyScenario(const edm::ParameterSet& scenario) {
  ...
  this->decodeMovements_(theScenario, btlBarrel, "BTLBarrel");
  // Endcap
  const auto& etlEndcaps = theAlignableMTD->ETLEndcaps();
  this->decodeMovements_(theScenario, etlEndcaps, "CSCEndcap");
  //¿¿¿¿¿¿DT sectors?????
...
void MTDScenarioBuilder::moveDTSectors(const edm::ParameterSet& pSet) {
  const auto& BTLmodules = theAlignableMTD->DTModules();
  //Take parameters
  ...
  for (const auto& iter : BTLmodules) {
    DTModuleId myId(iter->geomDetId().rawId());
  ...
  for (int wheel = 0; wheel < 5; wheel++) {
    for (int sector = 0; sector < 12; sector++) {
      ...
      for (int station = 0; station < 4; station++) {
        Alignable* myAlign = BTLmodules.at(index[wheel][station][sector]);
        this->moveModuleInSector(myAlign, disp, rotation, errorDisp, errorRotation);
        if (sector == 3 && station == 3) {
          Alignable* myAlignD = BTLmodules.at(index[wheel][station][12]);
          this->moveModuleInSector(myAlignD, disp, rotation, errorDisp, errorRotation);
        } else if (sector == 9 && station == 3) {
          Alignable* myAlignD = BTLmodules.at(index[wheel][station][13]);
          this->moveModuleInSector(myAlignD, disp, rotation, errorDisp, errorRotation);
        }
      }
    }
...
void MTDScenarioBuilder::moveCSCSectors(const edm::ParameterSet& pSet) {
  const auto& CSCchambers = theAlignableMTD->CSCModules();
  //Take Parameters
...
void MTDScenarioBuilder::moveMuon(const edm::ParameterSet& pSet) {
  const auto& BTLbarrel = theAlignableMTD->BTLBarrel();
  const auto& ETLendcaps = theAlignableMTD->ETLEndcaps();
  //Take Parameters
...
//______________________________________________________________________________________________________
void MTDScenarioBuilder::moveModuleInSector(Alignable* chamber,
