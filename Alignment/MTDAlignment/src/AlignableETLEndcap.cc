...
AlignableETLEndcap::AlignableETLEndcap(const std::vector<AlignableETLModule*>& etlModules)
    : AlignableComposite(etlModules[0]->id(), align::AlignableETLEndcap) {
...
std::ostream& operator<<(std::ostream& os, const AlignableETLEndcap& b) {
  os << "This ETL EndCap contains " << b.theETLModules.size() << " ETL modules" << std::endl;
...
Alignments* AlignableETLEndcap::alignments(void) const {
...
AlignmentErrorsExtended* AlignableETLEndcap::alignmentErrors(void) const {
