...
AlignableETLModule::AlignableETLModule(const GeomDet* geomDet) : AlignableDet(geomDet, false) {
  // even though we overload alignableObjectId(), it's dangerous to
  // have two different claims about the structure type
  theStructureType = align::AlignableETLModule;

  //////// Probably we don't need this in the ETL, the crystals are unitDets
...
std::ostream& operator<<(std::ostream& os, const AlignableETLModule& r) {
  const auto& theDets = r.components();

  os << "    This ETLModule contains " << theDets.size() << " units" << std::endl;
