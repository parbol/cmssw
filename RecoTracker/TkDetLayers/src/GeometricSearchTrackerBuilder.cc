...
#include "RecoMTD/DetLayers/plugins/BTLDetLayerGeometryBuilder.h"
#include "RecoMTD/DetLayers/plugins/ETLDetLayerGeometryBuilder.h"
...
GeometricSearchTracker* GeometricSearchTrackerBuilder::build(const GeometricDet* theGeometricTracker,
  ...
  //Tracker part
  GeometricSearchTracker *theSearchTrack = this->build(theGeometricTracker, theGeomDetGeometry, tTopo);

  theSearchTrack->addDetLayerGeometry();
  theSearchTrack->mtdDetLayerGeometry->buildLayers(*mtd);
  ...
  for(auto &&e : theSearchTrack->mtdDetLayerGeometry->allBackwardLayers()) {
      ...
      if(p) {
          ...
          //theSearchTrack->theMTDForwardMap.insert(std::pair<DetId, ForwardDetLayer const *>(id, p));
      }
  }
